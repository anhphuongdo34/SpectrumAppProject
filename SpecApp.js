...
function initializeApp() {
	//This function gets everything started by getting and loading the SpecInfo.xml file and using that data to create the Compounds menu.
	getXML("Spectra/SpecInfo.xml", loadXML, "New message!\n\n");
}

function xhrSuccess() {
	this.callback.apply(this, this.arguments);
} // This runs the callback function if the loadXML is successful.

function xhrError() {
	console.error(this.statusText);
} // This runs if the loadXML is not successful.

function getXML(
	sURL,
	loadXMLCallback /*, argumentToPass1, argumentToPass2, etc. */
) {
	// This function loads an XML file.  For this application the XML file contains information about each compound in the Spectroscopy Tutorial.  Change the XML file and you can load different compounds and information.
...
function loadXML() {
	// This is the callback function for the getXML function.  After the XML file is read in it is sent here.  The data in the XML file is used to create the Compounds menu.  After this the first spectrum and molecule are loaded into their respective canvases.
...
function makeSpecNameArray(xmlResponse) {
	// This creates a key/value array where the key is the compound's name as seen in the Compound menu and the value is the name as found in the spectrum jcamp file name.
	var xmlSpecName = xmlResponse.evaluate(
		"/SpectroscopyInfo/Compound/SpecName",
		xmlResponse,
		null,
		XPathResult.ORDERED_NODE_ITERATOR_TYPE,
		null
	);
	var xmlName = xmlResponse.evaluate(
		"/SpectroscopyInfo/Compound/Name",
		xmlResponse,
		null,
		XPathResult.ORDERED_NODE_ITERATOR_TYPE,
		null
...
function makeUnorderedList(listID) {
	// This is creating the html list that is used to make the Compound menu.
	var nameArray = [];
...
function add_li(nameArray, listID, list, i) {
	// Creates the list item and attaches an onclick function to it so that selecting an item from the list causes the corresponding spectrum file to be loaded.
	...
	A.onclick = function () {
		getAndShow(compoundString, spectrumString);
	};
...
function makeNameArray(nameArray) {
	//This creates an array containing the names of all the compounds.
	var xmlName = xmlResponse.evaluate(
		"/SpectroscopyInfo/Compound/Name",
		xmlResponse,
		null,
...
function getAndShow(compoundName, spectrumName) {
	// This function loads spectrum and molecule data from an external file then displays the spectrum and molecule in their respective divs.
	specName = compoundName;
	if (spectrumName == "Init") {
		// For when the app is first starting up.
	...
	if (spectrumName == "Init") {
		// First time through we need to display general info about the compound.
		document.getElementById("fragCell").innerHTML = "";
		var xPathExpr =
			"//Compound[SpecName='" + compoundName + "']/Description";
		var xmlMolDesc = xmlResponse.evaluate(
			xPathExpr,
			xmlResponse,
			null,
		...
		// Subsequent times through we need to display information about the specific spectrum.
		var xPathExpr =
			"//Compound[SpecName='" +
			compoundName +
			"']/" +
			whichSpectrum +
			"Description";
		var xmlPeakDesc = xmlResponse.evaluate(
			xPathExpr,
			xmlResponse,
			null,
			XPathResult.STRING_TYPE,
...
function makePeakArrays(compoundName) {
	// This function reads data from SpecInfo.xml for the current spectrum and puts it into an array.  This info will then be displayed when hovering over peaks in the spectrum or atoms in the molecule.
	var peakDescString = whichSpectrum + "Description";
	var xPathExpr =
		"//Compound[SpecName='" + compoundName + "']/" + peakDescString;
	var xmlPeakDesc = xmlResponse.evaluate(
		xPathExpr,
		xmlResponse,
		null,
	...
	peakNameArray = {};
	while (getList) {
		// iterate through all the peak descriptions for this spectrum in the SpecInfo.xml file.
...
function getSpectrum(spectrumFile, callback) {
	// This gets the spectrum data from the external file.
...
function showSpectrum(spectrumData) {
	// This creates the Structure Spectrum Set and loads the spectrum and molecule into their respective canvases in their divs.
	var spectrumSet =
		new ChemDoodle.io.JCAMPInterpreter().makeStructureSpectrumSet(
			"spec",
			spectrumData,
			200,
			200,
...
function removeCanvas(divId, canvasId) {
	// This function removes a canvas element from a div to clear out that div before adding a new spectrum or molecule.
...
function loadCanvas(divId, canvasId) {
	// This function loads a spectrum or molecule into its respective div.
	switch (divId) {
		case "specSpec":
...
function textDescribe(Description) {
	// This function takes text from a text file and displays it in a div.  Used to display information relevant to the spectra.
	var textDescription = document.getElementById("specDesc");
	textDescription.innerHTML = Description;
}

function showSomething(hovered) {
	// This function changes the display of text when hovering over a peak in a spectrum or the corresponding atom in the molecule.
...
function removeSomething() {
	// This function is for removing text from being displayed if not hovering over a peak or atom.
	var textDescription = document.getElementById("specDesc");
	textDescription.innerHTML = null;
}

function loadFragment(fragImage) {
	//This function loads an image of a molecule fragment into the Fragment Space on the web page.
...
function makeFragmentTable(resizeCallback) {
	// This function creates a 7x5 table and loads molecule fragment images into it.
	...
	cell.innerHTML =
		"<b>Click on a fragment to add it to the molecule's Fragment Space</b>"; //These first 7 or so lines of code create the table and a table header.
	...
	for (var i = 0; i < 5; i++) {
		...
		for (var j = 0; j < 7; j++) {
			...
			if (counter < 34) {
				...
				fragImage.onclick = function () {
					loadFragment(this);
				}; //Make it so that clicking on the fragment image loads it into the Fragment Space area of the page.
...
function resize() {
	//This just reads in the width and height of the image, cuts them in half, then assigns those new values to the width and height attributes.
...
function showFragTable() {
	//Makes the "hidden" table visible while also reducing the size of all the images in the table.
	var fragDiv = document.getElementById("fragTable");
	for (var i in fragArray) {
		//This loops runs through the whole array of fragment images and reduces their size.
		fragImage = fragArray[i];
		if (fragImage.width == fragImage.naturalWidth) {
			//checks to make sure the image hasn't already been resized.  Don't want to keep making them smaller.
			w = fragImage.width;
			w = w * 0.5;
			fragImage.width = w;
		}
...
function loadIRmovieFiles(peakName) {
	// Not using this yet.  It will load pdb files for displaying IR vibrations.
	// 			var movieArray = [];
	var frame;
	var frameName;
	for (var i = 0; i < 11; i++) {
...
function showVibration(peakName) {
	// Not using this yet.  It will put together all of the pdb files to display a movie of IR vibrations.
	// 			var movieArray = [];
...
function listAllProperties(o) {
	//this is just here so if I'm testing I can tell what properties some object has.
	var objectToInspect;
	var result = [];
	for (
		objectToInspect = o;
...
function showProps(obj, objName) {
	//also for testing purposes, shows property name and value.
	var result = "";
	for (var i in obj) {
...
function fileExists(url) {
	// This function checks to see if a file is actually on the server before downloading it.
	if (url) {
